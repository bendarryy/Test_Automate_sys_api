...
class InventoryItemViewSet(viewsets.ModelViewSet):
    ...
    def low_stock(self, request, system_id=None):
        ...
        products = Product.objects.filter(system=system, stock_quantity__lt=models.F('minimum_stock'))
    ...
    def expiring_soon(self, request, system_id=None):
        ...
        products = Product.objects.filter(system=system, expiry_date__isnull=False, expiry_date__lte=cutoff)
        serializer = self.get_serializer(products, many=True)
        return Response(serializer.data)
    ...
    def stock_history(self, request, system_id=None):
        ...
        stock_changes = StockChange.objects.filter(product__system=system).order_by('-created_at')  # Use 'created_at' instead of 'timestamp'
